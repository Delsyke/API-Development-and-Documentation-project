...
class TriviaTestCase(unittest.TestCase):
    ...
    # GET '/questions/<int:page>' ------> DONE
    def test_get_all_questions_default(self):
        res = self.client().get('/questions') #without page parameter
        data = json.loads(res.data)
        self.assertEqual(data['success'], True)
        self.assertEqual(res.status_code, 200)
        self.assertTrue(data['categories'])
    ...
    def test_get_all_questions_in_page(self):
        res = self.client().get('/questions?page=2') # with page parameter
        data = json.loads(res.data)
        self.assertEqual(data['success'], True)
        self.assertEqual(res.status_code, 200)
        self.assertTrue(data['categories'])
    ...
    def test_get_all_questions_404(self):
        res = self.client().get('/questions?page=9') #page out of range
        data = json.loads(res.data)
        self.assertEqual(data['success'], False)
    ...
    # POST '/questions' (to add new question) -------> DONE
    ...
    def test_add_question_incomplete_request_400(self):
        ...
        'answer': 'test answer2',
        'difficulty': 1 #No category in request body, Bad request
        }
        res = self.client().post('/questions', json=new_q)
        data = json.loads(res.data)
        self.assertEqual(data['success'], False)
    ...
    def test_add_question_invalid_request_400(self):
        ...
        'answer', 'test answer2',
        'difficulty', 1,
        'category', 3, #request body is a list (not dictonary}, Bad request
        ]
        res = self.client().post('/questions', json=new_q)
        data = json.loads(res.data)
        self.assertEqual(data['success'], False)
        self.assertRaises(TypeError) # checking for the exception raised
        self.assertEqual(res.status_code, 400)
        self.assertEqual(data['message'], 'Bad request')


    # POST '/questions' (to search) -------> DONE
    ...
    def test_search_question_invalid_request_400(self):
        chars = [{
        "searchTerm":'title'
        }] #search term is a list (not dictionary), Bad request'
        res = self.client().post('/questions', json=chars)
        data = json.loads(res.data)
        self.assertEqual(data['success'], False)
        self.assertEqual(res.status_code, 400)
    ...
    # DELETE '/questions/<question_id>' -------> DONE
    ...
    def test_delete_question_404(self):
        res = self.client().delete('/questions/10000') #non-existing id
        data = json.loads(res.data)
        self.assertEqual(res.status_code, 404)
        self.assertEqual(data["success"], False)
        self.assertEqual(data['message'], 'The resource could not be found')
    ...
    # GET '/categories/<int:category_id>/questions' ------> DONE
    ...
    def test_play_trivia_404(self):
        quiz = {
            ...
            'quiz_category': {
            'type': 'Soccer', 
            'id': '100' #non-existent category and id
            }
        }
        res = self.client().post('/quizzes', json=quiz)
        data = json.loads(res.data)
    ...
    def test_play_trivia_invalid_request_400(self):
        quiz = [{
            ...
            'quiz_category': {
            'type': 'Sports', 
            'id': '6' # Invalid request (list not dict object)
            }
        }]
        res = self.client().post('/quizzes', json=quiz)
        data = json.loads(res.data)
