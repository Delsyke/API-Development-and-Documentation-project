...
def paginate(question_list):
  ...
  # if len(question_list) % QUESTIONS_PER_PAGE != 0:
  #   total_pages = (int(len(question_list)/QUESTIONS_PER_PAGE) + 1)
  # else:
  #   total_pages = int(len(questions)/QUESTIONS_PER_PAGE)
...
def create_app(test_config=None):
  ...
  def internal_server_error(error):
    return jsonify ({
      "success": False,
      "error code": 500,
      "message" : "A problem occured while processing the request"
      }), 500
